...
Perfect for viral YouTube content creation!
"""
...
class BrainStateCalibrator(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("🧠 Brain State Calibrator - VIRAL EDITION")
        ...
        self.target_samples = 30  # Faster for viral content
    ...
    def init_ui(self):
        ...
        # Samples counter
        self.samples_label = QLabel("0 / 30 samples")
        ...
        for i, (name, key) in enumerate(states):
            ...
            states_grid.addWidget(button, 0, i)
        ...
        self.log("📹 Perfect for viral YouTube content!")
    ...
    def connect_device(self):
        ...
        try:
            ...
            if self.streamer.setup_board():
                ...
                self.status_label.setText("✅ Connected - Click a brain state to train")
    ...
    def select_brain_state(self, state_key):
        ...
        # If button was unchecked, stop training
        if not self.state_buttons[state_key].isChecked():
            self.current_class = None
            self.class_label.setText("Select a brain state")
            return
        ...
        # Reset sample accumulator for new class
        ...
        self.collection_timer.start(400)  # Collect every 400ms (faster)
    
    def collect_sample(self):
        """Collect a training sample by accumulating data over multiple calls."""
        if not self.streamer or not self.current_class:
            return
        
        try:
            # Get any available new data
            new_eeg_data, timestamps = self.streamer.get_new_data()
            
            if new_eeg_data is not None and new_eeg_data.shape[1] > 0:
                # Initialize sample accumulator if needed
                if not hasattr(self, 'sample_accumulator'):
                    self.sample_accumulator = []
                if not hasattr(self, 'target_samples_for_window'):
                    self.target_samples_for_window = self.streamer.sampling_rate  # 1 second of data
                
                # Add new data to accumulator
                for i in range(new_eeg_data.shape[1]):
                    self.sample_accumulator.append(new_eeg_data[:, i])
                
                # Check if we have enough data for processing (1 second)
                if len(self.sample_accumulator) >= self.target_samples_for_window:
                    # Convert to array and process
                    window_data = np.column_stack(self.sample_accumulator[-self.target_samples_for_window:])
                    ...
                    # Store sample (flatten band powers to 1D feature vector)
                    band_powers_flat = band_powers.flatten()  # Convert (channels, bands) to 1D
                    self.training_data[self.current_class].append(band_powers_flat)
                    self.samples_collected += 1
                    
                    self.log(f"📊 Sample {self.samples_collected}: {self.current_class} (accumulated {len(self.sample_accumulator)} total)")
                    self.update_progress()
                    
                    # Clear old data (keep only recent 2 seconds)
                    keep_samples = self.streamer.sampling_rate * 2
                    if len(self.sample_accumulator) > keep_samples:
                        self.sample_accumulator = self.sample_accumulator[-keep_samples:]
                    ...
                    if self.samples_collected >= self.target_samples:
                        ...
                        self.state_buttons[self.current_class].setText(f"✅ {self.current_class} ({self.target_samples})")
                        
                        self.log(f"✅ Completed {self.current_class} training!")
                        self.class_label.setText("✅ Class completed! Select next one")
                        self.current_class = None
                        self.sample_accumulator = []  # Reset accumulator
                        
                        # Check if enough states are trained
                        completed_states = len([s for s in self.training_data.values() if len(s) >= self.target_samples])
                        if completed_states >= 2:  # Need at least 2 states for classification
                            self.train_classifier()
                else:
                    # Update status with accumulation progress
                    progress_pct = (len(self.sample_accumulator) / self.target_samples_for_window) * 100
                    self.log(f"📈 Accumulating: {len(self.sample_accumulator)}/{self.target_samples_for_window} samples ({progress_pct:.1f}%)")
            else:
                self.log("⏳ Waiting for EEG data...")
            
        except Exception as e:
            self.log(f"❌ Error collecting sample: {e}")
            # Continue trying
            import traceback
            self.log(f"📋 Debug: {traceback.format_exc()[:200]}")
    ...
    def train_classifier(self):
        ...
        try:
            ...
            class_idx = 0
            for state, samples in self.training_data.items():
            ...
            if len(X_train) > 0:
                ...
                joblib.dump(model_data, 'brain_state_model.pkl')
                
                self.log(f"🎉 Training completed! Accuracy: {accuracy:.1%}")
                self.log(f"📊 Classes trained: {', '.join(class_names)}")
                self.status_label.setText(f"🎉 VIRAL MODEL READY! Accuracy: {accuracy:.1%}")
                
                # Show completion message
                QMessageBox.information(self, "🎉 Success!", 
                    f"Brain state classifier trained!\n\n"
                    f"Accuracy: {accuracy:.1%}\n"
                    f"Classes: {', '.join(class_names)}\n\n"
                    f"Model saved as 'brain_state_model.pkl'\n"
                    f"Ready for viral content! 🚀")
...
if __name__ == "__main__":
    ...
    print("🧠 Brain State Calibrator - VIRAL EDITION")
    print("Perfect for YouTube content creation!")
